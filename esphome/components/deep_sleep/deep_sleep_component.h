...
class DeepSleepComponent : public Component {
 public:
  ...
  /** Set the pin to wake up to on the nRF52 once it's in deep sleep mode.
   * Use the inverted property to set the wakeup level.
   */
  void set_wakeup_pin(InternalGPIOPin *pin) { this->wakeup_pin_ = pin; }
  void set_wakeup_pin_mode(WakeupPinMode wakeup_pin_mode) { this->wakeup_pin_mode_ = wakeup_pin_mode; }
#endif  // USE_NRF52
